 ...
 * * Fichier : class_client_io.php
 * * Note :	This work is licensed under a Creative Commons Attribution 2.5 Generic License http://creativecommons.org/licenses/by/2.5/
 *
 **/
class CRMClientIO extends ObjectPrint{
/**
 * CRMClientIO.GetDataFromImportedFile() -> void
 *
 ...
 **/
	protected static function GetDataFromImportedFile(){
		...
		if(Stream::Extension($file) == 'xml'){
			$options->data = $options->data['clients']['client'];
		}
		...
		foreach($config as $key => $value){
			...
			switch(trim(strtolower($value))){
				default:
					...
					if(self::IsMail(trim($value))){//Les dates sont ignorés
...
/**
 * CRMClientIO.CreateInstance() -> CRMClientIO
 **/	
 ...
 **/	
	public static function Export($clauses = '', $options = ''){
		...
		$obj->data = 	CRMClient::GetList($clauses, $options);
		$obj->clauses = $clauses;
		$obj->options = $options;
		...
		for($i = 0; $i < $obj->data['length']; $i++){
			$client = new CRMClient($obj->data[$i]);
		...
		switch($format){
			...
			case 'xml':
				...
				Stream::WriteXml($file, $Node, 'Généré CRM Javalyss');
...
/**
 * CRMClient#printPDF([options]) -> void
 **/
	public function printPDF($options = ''){
		...
		self::SetLink(System::Path('prints').'crm-clients-'.User::Get()->User_ID.'.pdf');
		//
		// Libération des ressources
		// 
		@unlink(self::GetLink());	
		
		$models = new ModelPDF(CRM_PATH . 'models/prints/clients/', System::Path('self'));
...
/**
 * CRMClient#PrintList([clauses [, options]]) -> void
 **/	
	public static function PrintList($clauses = '', $options = ''){
		...
		//
		//
		self::SetLink(System::Path('prints').'listing-clients-'.User::Get()->User_ID.'.pdf');
		//
		// Libération des ressources
		// 
		@unlink(self::GetLink());	
		
		$models = new ModelPDF(CRM_PATH . 'models/prints/listing/clients/', System::Path('self'));
