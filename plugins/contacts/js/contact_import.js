 ...
 **/	
	open:function(win){
		try{
			...
			panel.PanelSwip.addPanel($MUI('Contacts'), this.createPanel(win));
			//
			//
			//
			$S.fire('contact.import:open', win);
 ...
 **/	
	createPanel:function(win){
		...
		panel.appendChild(new Node('H1', {style:'margin: 0px 0px 25px;'}, $MUI('Importation des contacts')));
		//
		//
		//
		forms.File = new FrameWorker({
			multiple:	false,
			parameters: 'cmd=contact.get.data',
			mini:		true
		});
		...
		forms.File.on('complete', function(obj){
			...
			forms.Data = System.Contact.Import.createTable();
		...
		table.addHead($MUI('Ecraser le contact si il existe déjà') + ' ?').addCel(forms.EraseIfExists).addRow();
		table.addHead($MUI('Ajouter le contact si il existe déjà (doublon)') + ' ?').addCel(forms.CreateIfExists).addRow();
		
		panel.appendChild(table);
		
		panel.select('.show-import-elements').invoke('hide');
		
		forms.submit = 	new SimpleButton({text:$MUI('Importer mes contacts')});
		forms.submit.on('click', function(){
			System.Contact.Import.submit(win, forms);
		});
		
		win.Panel.PanelSwip.Footer().appendChild(forms.submit);
		
		win.submitContacts = forms.submit;	
 ...
 **/	
	submit:function(win, forms){
		...
		System.exec('crm.contact.import', {
			...
			onComplete:function(result){
				...
				var table = new TableData();
				table.addHead($MUI('Nombre de contact importée') + ' : ').addCel(obj.created, {style:'font-weight:bold'}).addRow();
				table.addHead($MUI('Nombre de contact fusionnée') + ' : ').addCel(obj.merged.length, {style:'font-weight:bold'}).addRow();
				table.addHead($MUI('Nombre de contact non importée') + ' : ').addCel(obj.noneAdd.length, {style:'font-weight:bold'}).addRow();
