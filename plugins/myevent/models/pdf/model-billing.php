<?php
/** section: MyEvent
 * class MyEventBillingPDF < pdfSimpleTable
 *
 * Modèle PDF de base.
 **/
class MyEventBillingPDF extends MyEventCommandPDF{
	
	static function Initialize(){
		$pdf = new self();
		$pdf->Output(MyEventCommand::GetLink(), 'F');	
	}
/**
 * new modelProforma()
 **/
	function __construct(){
		...
		$this->drawSectionTitle('Facture N°' . $command->Command_NB);
	...
	public function drawInfo($x = 125){
		...
		$command = 	MyEventCommand::Get();
		$wallet =	new MyWalletCard((int) $command->Wallet_Card_ID);
		...
		$this->SetFont('Arial','B', 9);
		$this->Cell(33, 6, utf8_decode('Date facture'), 0, 0, 'L');
		$this->SetFont('Arial','', 9);
		$this->Cell(50, 6, utf8_decode(ObjectPrint::DateFormat($command->Date_Confirm, '%d/%m/%Y')), 0, 0, 'L');
		$this->Ln(4.5);
		$this->SetX($x + 2);
