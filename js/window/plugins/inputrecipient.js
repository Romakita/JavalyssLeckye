 ...
 *
 * <p class="note">Cette classe est une extension de Window JS. Pour l'inclure vous devez utiliser la méthode Import('plugins.inputrecipient').</p>
 *
 **/ 
var InputRecipient = Class.from(InputPopup);

InputRecipient.prototype = {
	className:	'wobject area-input input-recipient title show-button',
	link:		'',
	parameters:	'',
/**
 ...
 **/	
	push:function(mail){
		...
		if(mail.value){
			if(!mail.value.isMail()) return;
		}else{
			if(!mail.isMail()) return;
		}
		
		var li = 	new Node('li', {className:'background grey'}, mail.text || mail);
		li.mail = 	mail.value || mail;
 ...
 **/
	draw: function(array){
		...
		$A(array).each(function(data){
			...
			line.observe('mousedown', onMouse);
