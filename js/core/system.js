 ...
 **/
	initialize: function(){
		this.link = (window.location.href.split('?')[0] + '/ajax/connected').replace('index_admin.php', '');
 ...
 **/
		this.trace = function(){
			if(this.Meta('MODE_DEBUG')){
				return this.Terminal.trace.apply(this.Terminal, $A(arguments));
			}
		}.bind(this);
		//----------------------------
		//----------------------------
 ...
 **/
	close: function(){
		...
		}catch(er){if(window['console']){console.log(er)};return;}
	},
/**
 * System.connect() -> void
 *
 ...
 **/
	meta: function(key, value, callback){
		...
		$S.exec('system.meta.commit', {
			parameters:	'meta=' + Object.EncodeJSON(obj),
			onComplete:	callback
		});
		...
		switch(argv[0]){
			...
			case 'cron':
				...
				if(argv.length > 1){
					switch(argv[1]){
						case 'start':
							var parameters = {cmd: 'system.cron.start'};	
							break;
						case 'stop':
							var parameters = {cmd: 'system.cron.stop'};	
							break;
						case 'info':
							var parameters = {cmd: 'system.cron.statut'};	
							break;
						case 'list':
							var parameters = {cmd: 'system.cron.list'};	
							break;
					}
				}else{
					var parameters = {cmd: 'system.cron.statut'};	
 ...
 **/
	onInit: function(result){
		...
		try{
			...
			this.trace(er);	
			this.trace(result.responseText);
 ...
 **/
	start: function(){
		...
		if(!this.CRON_STARTED){
			var ajax = this.exec('system.cron.start', {
				onSuccess:function(){
					new fThread(function(){
